...
### Requirement: Inventory server uses sync helper primitives

The inventory server repository SHALL use the public server helper primitives for cursor timestamp parsing, changed/deleted row splitting, pull table response construction, status changed-table detection, latest cursor formatting, latest-cursor row selection, and push table validation.
