...
The `rusqlite` backend SHALL execute batch and internal transaction requests fully inside the worker without interleaving other requests.
...
- **WHEN** a Baresync plaintext SQLite database created by the SQLx backend exists
