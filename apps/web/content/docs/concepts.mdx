...
The sync contract starts as a TypeScript object created by `syncSchema`. It records package name, encoding, limits, synced tables, local-only columns, server-only columns, and table metadata extracted from Drizzle.
...
The exported helpers are `localSyncRowState` and `apiSyncRowState`. Diagnostics require local dirty tracking and warn when server-side `sync_updated_at` is missing.
