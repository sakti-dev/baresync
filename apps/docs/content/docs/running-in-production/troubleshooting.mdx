...
- **`dirty_count > 0`** → Local changes exist but haven't been pushed. Continue to step 2.
- **`dirty_count === 0`** → Local changes were pushed. Continue to step 3.
...
```ts
import { syncOutbox, syncCursors } from "@repo/sync-contract/schema";
