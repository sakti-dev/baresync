...
If you want the copyable starter with both implementation styles side by side, use [`examples/inventory-json-polling`](https://github.com/sakti-dev/baresync/tree/main/examples/inventory-json-polling). It shows the primitive repository path and the Drizzle helper-backed path in the same app so you can compare the tradeoffs directly.
...
- `defineSyncConfig`, `defineProtobufSyncConfig`, and sync column helpers for turning paired local/API Drizzle SQLite schemas into sync configs.
