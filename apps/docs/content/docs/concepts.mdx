...
The sync contract starts from `defineSyncConfig` or `defineProtobufSyncConfig`, which pair local synced schema and API synced schema modules. They record package name, encoding, limits, synced tables, local-only columns, server-only columns, and table metadata extracted from Drizzle.
