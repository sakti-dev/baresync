...
If you are developing inside the Baresync repo, add `path` to point at the local crates:
...
`baresync-core` appears in both `[dependencies]` and `[build-dependencies]` because the build script uses it to discover migration files.
...
The build script discovers `.sql` files in your migrations directory and generates a Rust function that embeds them into the binary.

Create `apps/app/src-tauri/build.rs`:
...
This produces `inventory_migrations()` — a function your `lib.rs` will call.
