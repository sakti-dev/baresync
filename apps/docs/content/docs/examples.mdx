...
Run those commands from the directory that contains `sync.config.ts`.

Generation emits table order and manifest files. For protobuf workspaces, export `protobufSyncGeneratorConfig` from the same `sync.config.ts` and let `baresync generate` pick it up automatically.
